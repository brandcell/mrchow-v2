	...
	class CartForm extends HTMLElement {
		...
		updateCartQty(item, newQty){
			...
			fetch(KROWN.settings.routes.cart_change_url, {
				...
				.then(() => {
                  let spb_section_id = document.querySelector('.shopify-section.cart-progress-bar');
                  return fetch('?section_id=' + spb_section_id.id.replace('shopify-section-', ''));
                })
				.then(response => response.text())
				.then(text => {
                  ...
                  let new_container = sectionInnerHTML.querySelector('#cb-spb');
                  let old_container = document.querySelector('#cb-spb');
                  let cart_items = document.querySelector('.cb-cart-items');
                  old_container.remove();
                  
                  cart_items.parentNode.insertBefore(new_container, cart_items);
